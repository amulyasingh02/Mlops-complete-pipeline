...
log_file_path=os.path.join('logs',"data_preprocessing.log")
filehandler=logging.FileHandler(log_file_path)
filehandler.setLevel("DEBUG")

my_formatter=logging.Formatter('%(asctime)s-%(name)s-%(levelname)s-%(message)s')
consolehandler.setFormatter(my_formatter)
...
def main(text_column='text', target_column='target'):
    ...
    try:
        ...
        logger.debug("Done with train data..Starting the test data process")
        test_processed_data = preprocess_df(test_data, text_column, target_column)
        logger.debug("Done with test data..")
